...
namespace EncodePreviewPlugin.Patches
{
    public static class VideoFileWriterPatch
    {
        ...
        public static void Log(string message)
        {
            try
            {
                string logFile = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "ymm4_encodepreview.log");
        ...
        public static void Apply(Harmony harmony)
        {
            try
            {
                ...
                var vfwType = AccessTools.TypeByName("YukkuriMovieMaker.VideoFileWriter.VideoFileWriter");
                if (vfwType == null)
                {
                ...
                foreach (var ctor in ctors)
                {
                    try
                    {
                        ...
                        Log($"Patched VideoFileWriter ctor: {ctor}");
                    }
                    catch (Exception ex)
                    {
                        Log($"Failed to patch ctor: {ex}");
                    }
                }
        ...
        public static void Constructor_Prefix(object __instance, object scene, object plugin, string path)
        {
            try
            {
                Log($"VideoFileWriter constructor called! path: {path}");
